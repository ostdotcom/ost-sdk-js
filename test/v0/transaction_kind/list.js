...
describe('services/v0/transaction_kind/list', function () {
  ...
  it('should pass when response data keys match', async function() {
    ...
    assert.deepEqual(Object.keys(response.data).sort(), ['client_id', 'result_type', 'transaction_types', 'meta', 'price_points', 'client_tokens'].sort());
    assert.equal(response.data.result_type, "transaction_types");
    assert.isAbove(response.data.transaction_types.length, 1);
    assert.deepEqual(Object.keys(response.data.transaction_types[0]).sort(), ['id', 'client_transaction_id', 'name', 'kind', 'currency_type', 'currency_value', 'commission_percent', 'status'].sort());
    ...
    assert.deepEqual(Object.keys(response.data.meta.next_page_payload).sort(), [].sort());
  ...
  it('should return promise', async function() {
    const response = transactionKindService.list();
